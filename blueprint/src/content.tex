...
\begin{lemma}[First variations of the Ricci and scalar curvatures]
  ...
  The \emph{scalar} statement is still on the model space $M = E$, where
  Definition \ref{def:scalar} used to live: it is phrased through
  \texttt{ricciE}, the Ricci form built on \emph{constant} fields, which only
  exist globally when $M = E$. That restriction is now removable, and the first
  step is taken here: \texttt{ricciFormOfMetric} is the Ricci form of a metric
  on \emph{any} manifold --- Definition \ref{def:ricci-form} for the
  Levi-Civita connection of $\g$, built on the global $C^2$ extensions of Lemma
  \ref{lem:global-extension} instead of constant fields --- and
  \texttt{innerE\_deriv\_eq\_of\_isRicciFlowAt'} carries the flow equation
  $\partial_t \g = -2\Ric$ across to it, by instantiating the flow on those
  extensions and using uniqueness of derivatives.

  The other ingredient, $\operatorname{tr}_\g(\partial_t\Ric) = \Delta R$,
  rests on the contracted second Bianchi identity, and \emph{that} is now
  proved in both contractions (Theorems \ref{thm:contracted-bianchi} and
