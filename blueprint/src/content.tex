...
\begin{lemma}[$\nab \Rm$ is tensorial in three of its four slots]
  ...
  \leanok
  \uses{lem:bianchi-second, def:curvature}
  $(\nab_{fX} \Riem)(Y,Z)W = f\,(\nab_X \Riem)(Y,Z)W$, and likewise additively.

  \textbf{Proved} (\texttt{CurvatureDeriv.lean}). Each of the four terms of
  ...
  This is what makes the \emph{divergence}
  $\operatorname{div}\Rm(Y,Z,W) = \sum_i \langle (\nab_{e_i}\Riem)(Y,Z)W, e_i
  \rangle$ well defined, the trace being over the direction slot; and the
  divergence is what the contracted second Bianchi identity, and hence $\Delta
  \scal$, are about.

  \textbf{The other three slots are not done, and are not the same problem.}
  Under $Y \mapsto fY$ the \emph{first} term picks up a Leibniz term,
  ...
  Mathlib's \texttt{ContMDiffAt.mlieBracket\_vectorField}. With it,
  \texttt{covCurvature\_smul\_snd} and \texttt{covCurvature\_add\_snd} give
  the $Y$ slot, and \texttt{covCurvature\_antisymm} hands over the $Z$ slot for
  nothing. So $\nab\Riem$ is tensorial in three of its four slots.

  \textbf{What is left is the $W$ slot}, which is the analogue of the
  curvature's third and needs the frame-and-globalise argument of
  Lemma \ref{lem:curvature-pointwise} one level up. After that, the trace over
  the direction slot against the fourth is $\operatorname{div}\Rm$, and the
  contracted second Bianchi identity is in reach.
\end{lemma}
