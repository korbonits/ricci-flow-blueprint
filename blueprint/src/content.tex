...
\begin{lemma}[$\nab^2$ and $\Delta$ for a section of a general bundle]
  ...
  \textbf{The fibre metric is on $T\M$ only.} The trace is over an orthonormal basis of
  $T_x\M$, and \texttt{OrthonormalBasis.sum\_apply\_self\_eq} traces a bilinear map into an
  \emph{arbitrary} normed space, so $V$ needs a norm on its fibres but no inner product for
  $\Delta$ to be frame-independent. Following mathlib's own advice, $V$ carries those norms
  as plain binders rather than a \texttt{RiemannianBundle} instance, which is reserved for
  bundles with a preexisting fibre topology such as $T\M$. The price is that the
  specialisation $V = T\M$ cannot be stated as a lemma --- the norms a
  \texttt{RiemannianBundle} puts on $T_x\M$ are not definitionally the ones a binder
  supplies --- and nothing needs it: $T\M$-valued statements use \texttt{Hessian.lean}
  directly.
\end{lemma}
