...
\begin{lemma}[$\nab \Rm$ is tensorial in its direction]
  ...
  Mathlib's \texttt{ContMDiffAt.mlieBracket\_vectorField}. So $Y$ and $Z$ are
  now unblocked, at one more derivative than the direction slot --- and
  \texttt{covCurvature\_antisymm} halves what is left of them, since
  $\nab\Riem$ inherits the antisymmetry of $\Riem$ in those two slots, so
  whatever is proved for $Y$ transfers to $Z$. $W$ still needs the
