...
\paragraph{What is still missing.} Mathlib does not yet prove that
\texttt{leviCivitaConnection} is $C^1$ when $\g$ is $C^2$ (the file says
``future PRs''). Without that, $\Ric$ of the canonical connection is not known
to be non-junk, so the function $\g \mapsto \Ric(\g)$ is definable but not yet
usable. Theorem \ref{thm:ricci-well-defined} is the bridge: the moment
smoothness lands, applying it to \texttt{leviCivitaConnection} turns every
existential below into an equation in $\g$ alone. Until then the road is
unchanged in shape and shorter by one \texttt{sorry}.
...
\begin{definition}[Ricci flow]
  ...
  regularity in $t$ is demanded of the connection. The function
  $\g \mapsto \Ric(\g)$ itself, which DeTurck's trick and the curvature
  evolution equations both require, waits on smoothness of
  \texttt{leviCivitaConnection} upstream (end of Chapter
  \ref{chap:levi-civita}).
...
\begin{theorem}[Hamilton 1982]
  \label{thm:hamilton}
  \uses{thm:normalized-convergence, lem:bianchi-first, def:sectional, thm:ricci-well-defined}
  ...
  $\Ric$ is junk; both were corrected in September 2026.) It carries no
