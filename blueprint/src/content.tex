...
\begin{lemma}[A section with vanishing covariant derivative at a point]
  ...
  \textbf{What remains is bookkeeping, not analysis}: additivity of $\nab^2$ in its section
  slot, its finite-sum version, and the identification of \texttt{laplacian} with the frame sum
  $\sum_j \nab^2_{b_j,b_j}$. Until that is assembled the maximum principle for sections of a
  non-trivial bundle cannot be stated, and that --- not the moving fibre metric of Theorem
  \ref{thm:max-moving-set} --- is what now stands between this repo and Lemma
  \ref{lem:pinching}.
\end{lemma}
