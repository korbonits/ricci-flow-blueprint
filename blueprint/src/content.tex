...
\begin{lemma}[Global extension of a tangent vector]
  ...
  \lean{RicciFlowBlueprint.exists_contMDiff_extension, RicciFlowBlueprint.forall_contMDiff_iff_forall_tangent}
  \leanok
  ...
  vector". What it does \emph{not} yet give is that $\Ric(X,X)(x)$ depends only
  on $X(x)$: that is tensoriality of the curvature's third slot on a general
  manifold, which is the one remaining obstruction to stating positive Ricci
  pointwise in $v$ as Chow--Liao--Qin do.
\end{lemma}
