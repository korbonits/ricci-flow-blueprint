...
\begin{lemma}[Affine reparametrisation]
  ...
  \textbf{What is left before $\exp$} is uniqueness of geodesics on a whole
  \emph{interval}, not merely near a point: Lemma \ref{lem:geodesic-unique} is local, and
  $\exp_x(v) = c(1)$ is well defined only once two runs agreeing at $0$ are known to agree at
  $1$. That is the usual clopen argument --- the agreement set is open by local uniqueness and
  closed by continuity into $TM$.
\end{lemma}
