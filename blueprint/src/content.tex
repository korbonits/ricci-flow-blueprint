...
\begin{lemma}[Differentiating the Koszul combination]
  ...
  $\Rm_A$ is what $\partial_t\Rm$ equals along a family of metrics, so with
  $h = \partial_t \g$ this is $\operatorname{tr}_\g(\partial_t\Ric)$; under
  the Ricci flow $h = -2\Ric$ and the two contracted Bianchi identities
  (Theorems \ref{thm:contracted-bianchi}, \ref{thm:div-ricci}) turn the
  right-hand side into $\Delta\scal$.
\end{lemma}
