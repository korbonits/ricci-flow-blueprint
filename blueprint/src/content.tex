...
\begin{theorem}[Hamilton--Ivey pinching]
  ...
  \uses{thm:max-tensor, lem:evolution-rm, lem:pinching-ode}
  ...
  \emph{(Cao--Zhu, Theorem 2.4.1.)} Both \emph{ends} of the argument are done:
  Lemma~\ref{lem:ivey-set} puts the normalised initial data into $K$ and reads
  the estimate back off it. What remains is the middle --- assembling
  Lemma~\ref{lem:ivey-scal} and Lemma~\ref{lem:ivey-boundary} into invariance
  of $K$ under the ODE; then $f^{-1}$ and its concavity, which is what makes
  $K$ convex and so is what Theorem~\ref{thm:max-tensor} consumes; and, for
  Hamilton's later improvement with the $\log(1+t)$ term, a form of
