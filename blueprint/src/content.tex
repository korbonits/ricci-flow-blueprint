...
\begin{lemma}[The first pair of slots factored through $\Lambda^2$]
  ...
  \textbf{Where this stops, and why.} The \emph{second} factorisation --- the
  curvature form $\Lambda^2 T_x\M \times \Lambda^2 T_x\M \to \R$, i.e.\ the
  curvature operator --- is deliberately \emph{absent}. It \emph{defines} without
  complaint, but every application of it fails to elaborate: \texttt{isDefEq} again
  enters the exterior algebra and does not return, at four times the heartbeats,
  with both wedges replaced by opaque variables, with the proof fields extracted as
  named theorems, and with only one of the two arguments applied. The culprit is
  localised: it is the \emph{nested} codomain
  $\Lambda^2 E \to_{\ell} (\Lambda^2 E \to_{\ell} \R)$, not $\Lambda^2$ itself,
  since $\Phi_{c,d}$ --- a linear map out of $\Lambda^2 E$ into $\R$ --- applies
  perfectly well. A definition whose only characterisation cannot be stated is
  precisely the hazard this blueprint guards against, so it is not landed unusable.
  The design lead for the next attempt: build the curvature \emph{operator}
  $\Lambda^2 \to \Lambda^2$ directly through the inner product that
  \texttt{Analysis/InnerProductSpace/ExteriorPower.lean} supplies, rather than a
  bilinear form; or go through a bundled bilinear-map type; or define it on the
  basis \texttt{OrthonormalBasis.exteriorPower} gives.
\end{lemma}
