...
\begin{lemma}[Constant sectional curvature determines $\Rm$]
  ...
  \leanok
  \uses{def:sectional, lem:curvature-symmetries, lem:bianchi-first}
  ...
  \textbf{Proved} (\texttt{ConstantCurvature.lean}). This is the bridge between
  the two ways the conclusion of Theorem \ref{thm:hamilton} gets stated. Ours,
  \texttt{AdmitsConstPositiveSecMetric}, fixes the sectional curvatures;
  Chow--Liao--Qin's \texttt{admitsConstantPositiveSectionalCurvature} fixes the
  full $(0,4)$ tensor. The lemma says they are the same hypothesis, so the
  divergence recorded in \texttt{notes/hamilton-statement-comparison.md} closes.
\end{lemma}
