...
\begin{lemma}[First variation of the Levi-Civita connection]
  ...
  \lean{RicciFlowBlueprint.inner_deriv_leviCivitaOfMetric_eq}
  \leanok
  ...
  \textbf{Proved}, with two hypotheses stated in exactly the form used. First,
  the space derivatives $X(\g(Y,Z))$ commute with $\partial_t$ for the three
  fields at hand: a joint-regularity statement about $(t,x) \mapsto \g(t)(x)$,
  which the pointwise-in-$t$ Definition \ref{def:ricci-flow} does not impose.
  Second, $t \mapsto \nab^t_X Y(x)$ is differentiable; this follows from the
  differentiability of $\g$ through the musical isomorphism and is not yet
  proved. The scalar form
  (\texttt{hasDerivAt\_inner\_leviCivitaOfMetric}) needs only the first.
\end{lemma}
...
\begin{lemma}[Evolution of the curvature tensor]
  ...
  \uses{def:ricci-flow, def:curvature, def:laplacian, lem:variation-connection}
