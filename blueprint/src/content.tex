...
\begin{lemma}[Global extension of a tangent vector]
  ...
  What these do \emph{not} yet give is that $\Riem(X,Y)Z$ at $x$ depends only on
  $Z(x)$. The route is now short and unobstructed: expand $Z$ in the smooth
  orthonormal local frame of \texttt{OrthonormalFrame.lean}, which holds near
  $x$; globalise the frame fields and the coefficient functions by the two lemmas
  above (both are now available); and apply $C^\infty(\M)$-linearity in the third slot
  (\texttt{curvature\_sum\_right}, \texttt{curvature\_smul\_right}). The
  coefficients at $x$ depend only on $Z(x)$, so the value does too. That is the
  one remaining step to stating positive Ricci pointwise in $v$, as
  Chow--Liao--Qin do; note it is genuinely a congruence statement and not
  \texttt{TensorialAt}, which demands the identities for merely differentiable
  sections and so cannot hold in the third slot.
\end{lemma}
...
\begin{definition}[Laplacian]
  \label{def:laplacian}
  \lean{CovariantDerivative.laplacian}
  \leanok
  \uses{def:hessian}
  ...
  $\sum_i B(e_i,e_i)$ is basis-independent for any bilinear $B$).
...
\begin{lemma}[First variations of the Ricci and scalar curvatures]
  ...
  \lean{RicciFlowBlueprint.hasDerivAt_ricciOfMetric, RicciFlowBlueprint.hasDerivAt_scalarCurvatureOfMetric'_of_isRicciFlowAt}
  \leanok
  ...
  under the flow. The scalar statement is on the model space $M = E$, where
  Definition \ref{def:scalar} lives: constant fields are global smooth
  sections there, which the second slot of $\Ric$ requires
  (\texttt{ricciE}, \texttt{ricci\_add\_right\_const}). What remains for
  $\partial_t R = \Delta R + 2|\Ric|^2$ is $\operatorname{tr}_\g(\partial_t
  \Ric) = \Delta R$ under the flow, the contracted second Bianchi identity,
  which needs the trace to commute with $\nab$ on the manifold.
\end{lemma}
...
\begin{lemma}[Evolution of the curvature tensor]
  ...
  \uses{def:ricci-flow, def:curvature, def:laplacian, lem:variation-connection, lem:variation-curvature, lem:bianchi-second, lem:metric-trace-variation, lem:variation-scalar}
...
\begin{theorem}[Hamilton--Ivey pinching]
  ...
  Lemma~\ref{lem:ivey-ode}. What remains is the transport to the flow: $f^{-1}$
  on $[-e^2,\infty)$ and its concavity, which together with concavity of the
  least-eigenvalue function make $K$ convex, and that convexity is exactly what
  Theorem~\ref{thm:max-tensor} consumes. For Hamilton's later improvement with
  the $\log(1+t)$ term one additionally needs a form of
  Theorem~\ref{thm:max-tensor} for a \emph{time-dependent} family $\{K_t\}$.
\end{theorem}
...
\begin{lemma}[Hamilton--Ivey for the curvature ODE]
  ...
  \leanok
  \uses{lem:pinching-ode, lem:ivey-scal, lem:ivey-set}
