...
\begin{lemma}[A section with vanishing covariant derivative at a point]
  ...
  \textbf{What is still missing is $\Delta N(x) = 0$.} The same construction with quadratic
  coefficients gives it, and only the \emph{trace} of the second derivative has to be
  prescribed, not the full Hessian, which keeps that step small too. Until it is done the
  maximum principle for sections of a non-trivial bundle cannot be assembled, and that --- not
  the moving fibre metric of Theorem \ref{thm:max-moving-set} --- is what now stands between
  this repo and Lemma \ref{lem:pinching}.
\end{lemma}
