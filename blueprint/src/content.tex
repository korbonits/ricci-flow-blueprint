...
\begin{lemma}[Evolution of the curvature tensor]
  ...
  each other, for which Lemma \ref{lem:bilin-ricci-identity} is the first
  ingredient.
\end{lemma}
...
\begin{lemma}[The Ricci identity for a bilinear form]
  ...
  \texttt{cov2Curvature\_eq\_hessian}: $\nab^2\Rm$ against the Hessian of the
  \emph{section} $\Rm(A,B)C$, with the same cancellation of the section's first
  derivative. Two things differ. $\Rm$ has three input slots and a vector
  output, so there will be four curvature terms rather than two --- one per
  input slot and one for the output, the last being the only place a section's
  Hessian differs from a function's. And the expansion needs \emph{no
  regularity of the connection at all}: where the bilinear case had to
  differentiate a scalar and so needed \texttt{mvfderiv} to be additive, here
  the only splitting is $\nab$ over a difference of sections, which is an axiom
  of a covariant derivative. What it does need is the differentiability of the
  four sections $\nab\Rm$ is built from, which is exactly what
  \texttt{contMDiff\_covCurvature} supplies.
\end{lemma}
