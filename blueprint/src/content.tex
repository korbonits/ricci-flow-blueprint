...
\begin{theorem}[Tensor maximum principle]
  ...
  Nor is the time-dependent \emph{set} $K_t$ done --- that is a separate generalisation,
  needed only for Hamilton 1999's $\log(1+t)$ improvement.
\end{theorem}
...
\begin{theorem}[The maximum principles with a moving metric]
  ...
  \textbf{What is still not bought is a moving inner product on $V$}, and that is the
  distinction that matters: here the fibre is a fixed vector space with a fixed metric and
  only the base geometry evolves. Hamilton's curvature operator lives in $\Lambda^2T_xM$,
  whose inner product moves with $g_t$; trivialising \emph{that} is the content of
  Uhlenbeck's trick, and this theorem is what the trick is applied to.
\end{theorem}
