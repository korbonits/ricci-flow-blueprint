...
\begin{lemma}[Second-derivative test]
  ...
  \lean{RicciFlowBlueprint.laplacianFun_nonneg_of_isLocalMin_model}
  \leanok
  \uses{def:laplacian}
  At a local minimum $x_0$ of a $C^2$ function $f$, $\nab f(x_0) = 0$,
  $\nab^2 f(X,X)(x_0) \ge 0$ for every vector field $X$ differentiable at
  $x_0$, and $\Delta f(x_0) \ge 0$; for any covariant derivative, not only the
  Levi-Civita connection.

  \textbf{Proved on the model space} (a finite-dimensional normed space with
  its flat tangent bundle); the manifold statement is this one transported
  through a chart, since at a critical point the Hessian is
  connection-independent. With it, the abstract maximum principle
  \ref{thm:max-scalar} becomes the classical one for
...
\begin{proof}
  ...
  term and the connection term. The Laplacian is a sum of such terms over an
  orthonormal basis, whose extensions to sections are constant on the model
  space.
\end{proof}

\begin{lemma}[Evolution of the curvature tensor]
  \label{lem:evolution-rm}
  \uses{def:ricci-flow, def:curvature, def:laplacian}
...
\begin{theorem}[Scalar maximum principle]
  ...
  no manifold appear; once the Laplacian exists in Mathlib, the classical
  statement is this theorem plus the second-derivative test at a minimum.
\end{theorem}
...
\begin{theorem}[Tensor maximum principle]
  ...
  and the evolution equation \ref{lem:evolution-rm} supplying the
  differential inequality at a first touching point.
...
\begin{lemma}[Hamilton's curvature ODE in dimension three]
  ...
  $\lambda \ge \mu \ge \nu$, the Ricci eigenvalues are
  $\mu+\nu, \lambda+\nu, \lambda+\mu$, and the reaction term
...
\begin{lemma}[Pinching in dimension three]
  \label{lem:pinching}
  \uses{thm:max-tensor, lem:pinching-ode}
  ...
  ODE-invariant. Only the transfer is missing.
...
\begin{theorem}[Hamilton 1982]
  \label{thm:hamilton}
  \uses{thm:normalized-convergence, lem:bianchi-first, def:sectional, thm:ricci-well-defined, def:ricci-of-metric}
