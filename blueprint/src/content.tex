...
\begin{theorem}[Hamilton--Ivey pinching]
  ...
  maximum principle. Two things are new. The set is \emph{time-dependent}, so
  Theorem~\ref{thm:max-tensor} has to be restated for a family
  $\{K_t\}$ moving with the flow; and the defining function involves
  $\log$, so the Gr\"onwall comparisons of \texttt{Pinching.lean} are replaced
  by a differential inequality for $\scal + |\nu|\log|\nu|$.
\end{theorem}
