...
\begin{definition}[Parallel sections and geodesics]
  ...
  \textbf{What is still needed to \emph{solve} the equation} is the second-order
  content: in the fibre coordinates of a trivialisation, $\frac{D}{dt}V$ is
  $c' + \Gamma(\gamma')c$, so $\nab_{\gamma'}\gamma' = 0$ becomes $c'' =
  -\Gamma(c')(c')$, an ODE to which Picard--Lindel\"of applies. The tool for
  extracting it is \texttt{IsCovDerivAlong.eq\_sum\_of\_expansion}: $D/dt$ is
  computed by \emph{any} local expansion $V = \sum_i f_i\,(W_i\circ\gamma)$, so
  in particular by the coordinate frame.
\end{definition}
