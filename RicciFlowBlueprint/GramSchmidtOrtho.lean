/-
Ported into this project from Mathlib PR #26221 ("Mr. Covariant Derivatives"),
branch `grunweg/mathlib4:MR-covariant-derivatives`, which is open, blocked and
six months stale as of 2026-09-07. Upstream copyright and authorship below are
unchanged; the only edits are mechanical (the Lean module system is not used in
this project) plus the removal of one lemma that upstream marks unused and
leaves `sorry`. **Delete this file the moment #26221, or a split-out of it,
lands in Mathlib.**
...
theorem gramSchmidt_zero (n : ι) : gramSchmidt (0 : ι → (x : B) → E x) n = 0 := by
  ext x
  exact InnerProductSpace.gramSchmidt_zero ℝ n
...
lemma contMDiffWithinAt_aux
  ...
  have := (hs.inner_bundle ht).smul
    (ContMDiffWithinAt.inv₀ (hs.inner_bundle hs) (inner_self_ne_zero.mpr hs'))
  apply this.congr
  · intro y hy
    congr
...
theorem ContMDiffWithinAt.orthogonalProjection
  ...
  exact ContMDiffWithinAt.smul_section (contMDiffWithinAt_aux hs ht hs') hs
...
lemma gramSchmidt_contMDiffWithinAt (hs : ∀ i, CMDiffAt[u] n (T% (s i)) x)
  ...
  have : LinearIndependent ℝ ((fun x_1 ↦ s x_1 x) ∘ @Subtype.val ι fun x ↦ x ∈ Set.Iic i') := by
    apply hs'.comp aux
    intro ⟨x, hx⟩ ⟨x', hx'⟩ h
    exact Subtype.ext (congrArg (fun z : { y : ι // y ∈ Set.Iic i } ↦ (z : ι)) h)
