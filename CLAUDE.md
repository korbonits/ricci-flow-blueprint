...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **The geometric half of the identity is now proved too**:
   ...
   `tr_g(Δ_g h) = Δ(tr_g h)` — `hessianFun_traceBilin_eq` is that identity before the
   last trace, so what is missing is only a frame version of `laplacianFun`
   (`hessianFun` packaged as a bilinear form by `mkHom₂`, as `hessianAt` is for
   sections). Then `h = −2Ric` and the
