...
| `TensorPreservation.lean` | **proved**: **the maximum principles with a MOVING metric** — `le_of_laplacian_moving` and `mem_of_laplacian_moving` for `∂ₜu = Δ_{g_t}u + F t (u)` against a family `g : ℝ → ContMDiffRiemannianMetric I 2`. **A moving Laplacian costs nothing**, for the reason `ScalarPreservation.lean` found one level down: the principle inspects `Δ` only through `laplacianFun_nonneg_of_isLocalMin`, which holds for **each metric separately**, so the abstract `le_of_deriv_ge_at_min_time`/`mem_of_deriv_le_at_max_time` apply with the `letI RiemannianBundle` introduced *inside the proof*, one time at a time. Both compiled first try — the pattern is now routine. **What this does NOT buy**: the fibre `V` is a *fixed* inner product space. Hamilton's curvature operator lives in `Λ²T_xM`, whose inner product moves with `g_t`; trivialising that is Uhlenbeck's trick, and this theorem is what the trick is applied *to*. Note `[T2Space M]` is **not** needed (the tensor version's touching-point step is `laplacianFun_nonneg_of_isLocalMin`, not a frame argument) |
