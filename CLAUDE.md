...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Left on this line**: uniqueness of geodesics (mathlib's
   `ODE_solution_unique` on the same system), then `exp`, then parallel
