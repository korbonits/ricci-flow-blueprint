...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **The geometric half of the identity is now proved too**:
   ...
   with `tr_g(∂ₜRic)` — `derivCurvatureEndoE_apply_field` puts the inner trace's
   summand in `covTwoTensor` form on frame fields, so what is left is
   `traceCLM` as a frame sum (`LinearMap.trace_eq_sum_inner`) and
   `metricTraceE_eq_sum` for the outer one — and
