...
3. **`∂ₜ Rm = Δ Rm + Q`** (Uhlenbeck's trick) — still the single gate for both
   (1) and (2), but **both sides now exist on a general manifold**, and **both
   commutation bricks are in** (`RicciIdentity.lean`). **What the remaining work
   ...
   now the bulk**, and it is the one place a metric is unavoidable: the
   contraction is against `g`, so it will run on the same orthonormal-frame
   machinery `TraceCov.lean` and `Divergence.lean` use, with the differentiated
   second Bianchi identity supplying the antisymmetrised `∇²Rm`.
