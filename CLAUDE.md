...
- **Uhlenbeck's trick is gated by the SAME mathlib gap as the `L`-geometry line.**
  ...
  **And `C¹` dependence on a parameter is proved too** (2026-09-14,
  ...
  So the gap on this line is now only **`C^k` for `k ≥ 2`**, and the route to it
  is visible — `(Φ, D_xΦ)` satisfies the *linear* block-triangular system
  `∂ₜ(Φ,Ψ) = (AΦ, A'Φ + AΨ)`, so the `C¹` theorem applies to the augmented
  system and `k` follows by induction.
