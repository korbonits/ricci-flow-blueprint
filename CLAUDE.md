...
3. **`∂ₜ Rm = Δ Rm + Q`** — **CLOSED** (`CurvatureFlow.lean`,
   ...
   of `∇²h`. **What remains on this line is the flow specialisation only** —
   `isKoszulOf_derivDifference` + `innerE_deriv_eq_of_isRicciFlowAt'` under the
   two-step `letI`, the same bookkeeping `ScalarFlow.lean` did for the scalar
