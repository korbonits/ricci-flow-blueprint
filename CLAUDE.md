...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   this line:** the *contraction* itself. `div Rm` now **exists**
   (`Divergence.lean`), so what is left is the contracted second Bianchi
   identity (`div Ric = ½ d scal`), then `tr_g Ṙic = Δ scal` under the flow and
   hence `∂ₜ scal = Δ scal + 2|Ric|²`. `RicciVariation.lean` already has
