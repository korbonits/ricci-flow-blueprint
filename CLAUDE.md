...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Started 2026-09-08** (`CovariantAlongCurve.lean`): `velocity`,
   ...
   only through its germ, by the bump argument, not by fiat. **Next on this
   line is uniqueness, before existence**: uniqueness is what makes the
   interface safe to build on and it does not depend on the chart construction.
   It needs the local-frame expansion `V = ∑ᵢ fᵢ · (Eᵢ ∘ γ)` — locality plus
   the three axioms then pin `D V t` down — so `OrthonormalFrame.lean`'s
   `IsOrthonormalFrameOn` and the coefficient functions `fᵢ u = ⟪V u, Eᵢ(γ u)⟫`
   are the pieces to reach for.
