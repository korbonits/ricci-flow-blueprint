...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Next is Picard–Lindelöf** (`Analysis/ODE/`, `IsPicardLindelof`): `Γ` is
   already `C^k` (`contMDiffAt_christoffelCoord`), so what is left is packaging
   `(p,v) ↦ (v, −Γ(p)(v)(v))` as a first-order system on `E × E`, checking the
   Lipschitz hypothesis on a small ball, and transporting the solution back
   through the chart. Then `exp`, then parallel transport and Jacobi fields.
