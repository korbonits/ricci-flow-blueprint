...
- **Uhlenbeck's trick is gated by the SAME mathlib gap as the `L`-geometry line.**
  ...
  `ẋ = A(t)x` on all of `ℝ`, so the **global existence** obstacle is gone and the
  whole gap on this line is now exactly **`C^k` dependence on a parameter**.
