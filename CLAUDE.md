...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **Roadmap item 2 is closed.** What is left on this line is
   ...
   `tr_g Ṙic = Δ R` is the only missing piece, on no restricted `M`.
   ...
   **The geometric half of the identity is now proved too**:
   ...
   from `inner_derivDifferenceE_eq` (transferring its `extend` fields to
   general ones by the `covBilin` congr lemmas), identify `∑_{i,j}⟪Rm_A…⟫` with
   `tr_g(∂ₜRic)` through `hasDerivAt_ricciOfMetric` and
   `hasDerivAt_curvatureE_leviCivitaOfMetric`, and prove
   `tr_g(Δ_g h) = Δ(tr_g h)` (the trace lemma twice). Then `h = −2Ric` and the
   two contracted Bianchi identities give `Δ scal`.
