...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Next is existence**, and it is the only place a chart is unavoidable: over
   a trivialisation, `D V = e.symm(γ·, c' + Γ(γ')c)` with `Γ` the difference
   tensor of `cov` against the chart-flat connection — which is exactly what
   `Variation.lean`'s `differenceE` already builds. Then `∇_{γ'}γ' = 0` is an
   ODE in `c`, Picard–Lindelöf gives geodesics, and `exp` follows.
