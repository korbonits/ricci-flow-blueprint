...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **first contraction is proved** (`Divergence.lean`,
   `divCurvature_eq_covRicci_sub`: `div Rm(Y,Z,W) = (∇_Y Ric)(Z,W) −
   (∇_Z Ric)(Y,W)`). Left: the *second* contraction, `div Ric = ½ d scal`
   — trace that identity again over `Y` and `W`, which needs `Rm`'s pair
   symmetry carried through `∇` and `TraceCov.lean` to turn
   `∑ⱼ(∇_Z Ric)(eⱼ,eⱼ)` into `Z(scal)`. Then `tr_g Ṙic = Δ scal` under the flow
   and hence `∂ₜ scal = Δ scal + 2|Ric|²`. `RicciVariation.lean` already has
