...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **The geometric half of the identity is now proved too**:
   ...
   with `tr_g(∂ₜRic)` through `hasDerivAt_ricciOfMetric` and
   `ricciOfMetric_eq_trace`, and
   ...
   frame-argument-free `covOneFormAt` already is the packaging. Then
   `h = −2Ric` and the two contracted Bianchi identities give `Δ scal`.
