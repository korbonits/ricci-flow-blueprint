...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **The derivation, and why it is a `∇²h` problem.** With
   ...
   (`BilinDeriv.lean`) and so are `∇²h` and `Δ_g h` (`BilinLaplacian.lean`).
   **What is left on this line is `div div h`** (the other double trace, pairing
   slots 1&3 and 2&4 rather than 1&2), and then the identity itself.
