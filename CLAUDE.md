...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **`div div Ric = ½ Δ scal` is done too** (`ScalarEvolution.lean`). What is
   left is only the specialisation `h = −2Ric`: scaling `divDivBilin`,
   `laplacianBilin` and `traceBilin` in `h`, plus `traceBilin ricciForm =
   scalarCurvatureAt` — which is `rfl`, since `scalarCurvatureAt` is *defined*
   as that frame sum.
