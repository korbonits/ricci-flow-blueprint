...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Uniqueness is proved too** (`eventuallyEq_of_isGeodesic`,
   ...
   no `T2Space` on a bundle's total space. So `exp_x v = c 1` is now
   well defined. **Left on this line**: `exp` itself (a `IsGeodesicRun`
   package, the definition by choice, and homogeneity from reparametrisation),
   then parallel transport and Jacobi fields.
