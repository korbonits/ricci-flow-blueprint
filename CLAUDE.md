...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Uniqueness is proved too** (`eventuallyEq_of_isGeodesic`,
   ...
   boundarylessness**. **Left on this line**: uniqueness of geodesics on a whole
   *interval* — `eventuallyEq_of_isGeodesic` is only local, and `exp_x v = c 1`
   is well defined only once two runs agreeing at `0` are known to agree at `1`;
   that is the clopen argument (open by local uniqueness, closed by continuity
   into `TM`). Then `exp`, then parallel transport and Jacobi fields.
