...
| `CurvatureLaplacian.lean` | **in progress**: `cov2Curvature` = `∇²Rm`, with **one correction per slot of `∇Rm` — five terms**. `cov2Curvature_congr_dir` (pointwise in the outer direction) needs **no frame argument**: it is the four-slot pointwise lemma of `CurvatureDeriv.lean` cashed in — every term is a CLM at `X x` or `∇Rm` in a pointwise slot applied to a field whose value at `x` is determined by `X x`. `_smul_dir`/`_add_dir` and `_smul_snd`/`_add_snd` (the inner slot is the usual Leibniz cancellation; coefficient asked for `C³` because `(Xf)` must be `C²`). Regularity: `X,Y,A,B` `C³`, `C` `C⁴` — the last slot is expensive at every level, here because of `covCurvature_congr_fth`. **Left**: pointwise in `Y` (frame-and-globalise at `C³`), then `ΔRm` as the trace of the resulting bilinear map, frame-independent by `sum_apply_self_eq` |
