...
- Mathlib has **no maximal-solution ODE theory** — hence germ uniqueness in
  `Homogeneous.lean`.
- ~~No global `C²` extension of a tangent vector; Ricci is not a pointwise
  bilinear form; the scalar curvature is model-space only.~~ Closed:
  `GlobalExtension.lean` builds the extension, `CurvaturePointwise.lean` makes
  the curvature's third slot pointwise, and `RicciForm.lean` turns `Ric` into a
  continuous bilinear form and `scal` into a genuine trace on any manifold.
  **Still model-space only:** `∂ₜ R` (`RicciVariation.lean`), which is stated
  through `ricciE`/`scalarCurvatureOfMetric'` and has not been ported to
