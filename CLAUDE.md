...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **What is left is the chart transport**: `Γ` as a function of the chart
   coordinate `p` rather than of `y : M`, i.e. composing with
   `(extChartAt I x₀).symm`. **This is where boundarylessness enters** — for a
   general `ModelWithCorners` the chart target is not open in `E`, so
   `ContDiffAt` is the wrong predicate; add `[I.Boundaryless]` (there is
   precedent: `SecondDerivativeTest.lean` already restricts to a boundaryless
   `M`). Then the system is `(p,v) ↦ (v, −Γ̃(p)(v)(v))` on `E × E`, `C¹` because
   `Γ̃` is and evaluation of a bilinear map is smooth. Then `exp`, then parallel
   transport and Jacobi fields.
