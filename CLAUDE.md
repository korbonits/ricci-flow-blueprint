...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **The geometric half of the identity is now proved too**:
   ...
   alone. **What is left is the double trace**: apply it with `A = ∂ₜ∇`
   (through `inner_derivDifferenceE_eq`, transferring `extend` fields to
   general ones by the `covBilin` congr lemmas), then sum over a frame twice
   and collect — the arithmetic is `div div h − ½Δ tr h` from the first sum and
   `½Δ tr h` from the second.
