...
2. ~~**The manifold trace lemma** `X(tr_g B) = tr_g(∇_X B)`~~ — **done**,
   ...
   **What is left on this line is the identity itself**: trace the first
   variation of `Rm` twice against the Koszul formula for `∂ₜ∇`, which is a
   computation on objects that now all exist.
