...
4. **Perelman's `L`-geometry** (`def:reduced-volume`,
   ...
   **Next**: assemble the second-order equation. `coeffAlong γ (velocity γ) x₀ i`
   is `fun u ↦ b.repr (p' u) i` *near* `t` (needs `γ` `C¹` near `t`, not just
   differentiable at it, plus continuity into the chart source), so its `deriv`
   is `b.repr (p'' t) i` once `b.coord i` is pushed through `deriv`
   (`ContinuousLinearMap.deriv` / `.comp`). Then `repr_covAlong_eq_christoffel`
   reads `∇_{γ'}γ' = 0` as `p'' = −Γ(p')(p')` on an open subset of `E`, and
   mathlib's Picard–Lindelöf (`Analysis/ODE/`) applies — `Γ` is already `C^k`
   (`contMDiffAt_christoffelCoord`). Then `exp`, then Jacobi fields.
