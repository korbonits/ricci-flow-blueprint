...
3. **`∂ₜ Rm = Δ Rm + Q`** (Uhlenbeck's trick) — **now the single gate**, for
   ...
   The prerequisite `∇Rm` **as a tensor** is now half built: `CurvatureDeriv.lean`
   has `C^∞(M)`-linearity in **all four slots**, which is what makes the trace
   `div Rm` frame-independent. What is still missing is the *pointwise*
   statement (`covCurvature_congr_*`, the analogue of `curvature_congr_third`
   one level up) — the same frame-and-globalise build as
   `CurvaturePointwise.lean`, now with the four-slot linearity already in hand
   to feed it.
