...
3. **`∂ₜ Rm = Δ Rm + Q`** (Uhlenbeck's trick) — still the single gate for both
   (1) and (2), but **both sides now exist on a general manifold**, and the
   first commutation brick is in (`RicciIdentity.lean`: the Ricci identity for
   a bilinear form, one curvature term per slot of `h`). **What the remaining
   work decomposes into**, in order: (i) ~~the Ricci identity for `∇²h`~~ —
   **done**; (ii) the same for `∇²Rm`, i.e. curvature acting on the `(1,3)`
   tensor `Rm`, which is what lets the *differentiated* second Bianchi identity
   be antisymmetrised; (iii) the trace of that against the metric, turning
   `∇²Ric` into `ΔRm` plus quadratic terms; (iv) the substitution `h = −2Ric`
   into `inner_covTwoTensor_eq` and the bookkeeping to land on `Q`. Steps
   (ii)–(iii) are the bulk.
